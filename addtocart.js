...
async function musicUpdate(req, res) {
    try {
        ...
        const data = await dbClient.table("musicadd").where({ id: req.params.id }).update({ title, price, image, Description})
        res.json({
            status: true,
            success: true,
...
async function musicDelete(req,res){
    try{
      const data = await dbClient.table('musicadd').where({id: req.params.id}).delete();
...
async function Addtocart(req, res) {
  ...
  async function Buymusic(request, response){
    const email = request.params.email;
  ...
  module.exports=
{
   musicUpdate,musicDelete,Addtocart,getAddtocarts,Buymusic
