...
async function musicAdd(req, res) {
    try {
        ...
        const data = await dbClient.table("musicadd").insert({ title, price, image, Description})
        res.json({
            status: true,
            success: true,
            message: "music Added"
...
module.exports ={
    register,login,getUserByEmail,musicAdd,uploadImage,getAllmusic,updateUser,deleteUser
}
